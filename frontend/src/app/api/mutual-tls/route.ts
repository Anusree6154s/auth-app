...
export async function GET() {
  try {
    ...
    const clientOptions = {
      key: fs.readFileSync(path.join(certPath, "certs", "client.key")),
      cert: fs.readFileSync(path.join(certPath, "certs", "client.crt")),
      ca: fs.readFileSync(path.join(certPath, "certs", "ca.crt")), // CA that signed the server certificate
    };
